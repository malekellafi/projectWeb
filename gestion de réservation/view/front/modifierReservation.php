...
if(isset($_POST['id_rsv']) && isset($_POST['id_exec']) && isset($_POST['nbrPlaceDispo']) && isset($_POST['nbrPlace']) && isset($_POST['oldPlace']))
{
    $resC = new ReservationC();
    $oldNbr = $_POST['oldPlace'];
    $nbrExec = $_POST['nbrPlaceDispo'];
    ...
    $res = new Reservation(1,$_POST['id_exec'],$nbrRes,date("Y-m-d H:i:s"));
    $resC->modifierReservation($_POST['id_rsv'],$res);
    $resC->nbrPlaceSet($_POST['id_exec'],$nbrExec);
    header('Location: reservationList.php');
}
$exec = $execC->recupererExec($_GET['id_exec']);
$rsrv = $resC->recupererReservation($_GET['id']);
?>
<!doctype html>
<html lang="en">
    <head>
...
-->
  ...
  <form class="contact-form me-lg-5 pe-lg-3" action="modifierReservation.php" method="post" id="myForm">
    <div class="form-floating">
        <input type="number" name="nbrPlace" id="nbrPlace" class="form-control" value="<?php echo $rsrv['place']; ?>">
        <label for="nbrPlace">Nombre place</label>
        <span id="nbrError"></span>
    </div>
    <input type="text" name="id_exec" value="<?php echo $exec['id']; ?>" hidden>
    <input type="text" name="id_rsv" value="<?php echo $rsrv['id_rsv']; ?>" hidden>
    <input type="number" name="oldPlace" value="<?php echo $rsrv['place']; ?>" hidden>
    ...
    <div class="col-lg-5 col-6">
        <button type="reset" class="form-control">Annuler</button>
    </div>
</form>
<script>
    ...
    myForm.addEventListener("submit", function(e) {
        ...
        if(nbrPlace.value == "" || nbrPlace < 1 || !(/^[0-9]+$/.test(nbrPlace.value))){
